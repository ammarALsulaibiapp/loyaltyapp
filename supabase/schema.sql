-- LoyaltyPass Database Schema
-- Complete multi-tenant loyalty platform
...
-- Index for available rewards lookup
...
BEGIN
    ...
    INTO sequence_num
    FROM invoices
    WHERE TO_CHAR(created_at, 'YYYY-MM') = current_year || '-' || current_month;
    
    RETURN 'INV-' || current_year || current_month || '-' || sequence_num;
END;
$$ LANGUAGE plpgsql;

-- Function to check and unlock rewards automatically
...
-- Trigger to update customer totals after visit
...
-- Trigger to mark reward as redeemed
...
BEGIN
    ...
    SET 
        is_redeemed = TRUE,
        redeemed_date = NEW.redemption_date,
        redeemed_by = NEW.staff_id
...
-- Create default super admin (run after first user signs up)
-- You'll need to update this with actual user ID after first signup
-- Example: UPDATE profiles SET role = 'super_admin' WHERE email = 'admin@loyaltypass.com';
